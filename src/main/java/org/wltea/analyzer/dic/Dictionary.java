...
public class Dictionary {
	...
	/**
	 * 根据动态字典的配置文件读取动态配置扩展词并加载到主扩展词中。
	 * 
	 * @param dynamicDictFiles
	 */
	public static void loadDaynamicDictToMainDict(List<String> dynamicDictFiles){
		
		if(singleton == null){
			Dictionary.initial(DefaultConfig.getInstance());
		}
		
		if(dynamicDictFiles != null){
			InputStream is = null;
			
			//读取扩展词典文件
			for(String dynamicDictFile : dynamicDictFiles){
			
				is = Dictionary.class.getClassLoader().getResourceAsStream(dynamicDictFile);
				//如果找不到扩展的字典，则忽略
				if(is == null){
					continue;
				}
				...
				try {
					...
					System.err.println("Dynamic Dictionary loading exception.");
					ioe.printStackTrace();
